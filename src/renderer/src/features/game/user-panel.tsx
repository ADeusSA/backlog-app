...
import { GAME_STATUSES, MASTERABLE_STATUSES, OWNERSHIPS, RATEABLE_STATUSES, STORES } from '@shared/constants'
...
export function UserPanel({ game, ratingRef, hoursInputRef }: UserPanelProps): ReactElement | null {
    ...
    () =>
      Object.fromEntries(
        GAME_STATUSES.map((status) => [
          status,
          { title: t(`status.${status}`), description: t(`status.${status}.desc`) } satisfies StatusPickerLabel
        ])
  ...
  function setTime(nextHours: number, nextMinutes: number): void {
    const total = Math.max(0, Math.round(nextHours) * 60 + Math.max(0, Math.min(59, Math.round(nextMinutes))))
    ...
    >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="opacity-45">
                  <RatingStars value={userGame.rating} readOnly ariaLabel={t('game.panel.rating')} />
                </div>
              </TooltipTrigger>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {userGame.playtimeMode === 'sessions' ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="secondary" size="sm" disabled className="w-full justify-start">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Select<Ownership> value={userGame.ownership} onChange={(value) => patch('ownership', { ownership: value }, 0)}>
        ...
        {userGame.ownership === 'subscription' && (
          <Input
            ...
            onChange={(event) => patch('subscriptionService', { subscriptionService: event.target.value || null })}
          />
        )}
      </div>
      ...
      <Button variant="danger" size="sm" loading={removeBusy} onClick={() => void removeFromLibrary()} className="self-start">
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-center gap-1">
        ...
        />
        {value ? (
          <Button variant="icon" size="sm" onClick={() => onChange(null)} aria-label={clearLabel} title={clearLabel}>
            <X size={13} strokeWidth={1.75} />
          </Button>
        ) : (
          <Button variant="icon" size="sm" onClick={() => onChange(today())} aria-label={todayLabel} title={todayLabel}>
